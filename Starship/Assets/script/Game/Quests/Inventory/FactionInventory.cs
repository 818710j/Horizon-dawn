		...
		public class FactionInventory : IInventory
		{
			...
			void CreateHomeRegionItems()
			{
				...
				*/
				...
				var ships = _database.ShipBuildList.Playable().NormalShips().Where(item => item.Ship.SizeClass == SizeClass.Frigate).RandomUniqueElements(5 + random.Next(2), random);
				foreach (var ship in ships)
					_items.Add(_productFactory.CreateRenewableMarketProduct(_itemTypeFactory.CreateShipItem(new CommonShip(ship)), 1, _starId, Market.ShipRenewalTime, pricescale));
				...
				*/
				var components = _database.ComponentList.Available().Where(item => item.Level == 0).RandomUniqueElements(10 + 8 * extraGoods, random);
				foreach (var component in components)
					if (random.Percentage(20 + 20 * extraGoods))
						_items.Add(_productFactory.CreateRenewableMarketProduct(_itemTypeFactory.CreateComponentItem(ComponentInfo.CreateRandomModification(component, random, ModificationQuality.P4, ModificationQuality.P1)), random.Next(12 + 9 * extraGoods), _starId, Market.CommonComponentRenewalTime, 1f * pricescale));
					else
						_items.Add(_productFactory.CreateRenewableMarketProduct(_itemTypeFactory.CreateComponentItem(new ComponentInfo(component)), random.Next(8 + 2 * extraGoods), _starId, Market.CommonComponentRenewalTime, 1f * pricescale));
            ...
            void CreateItems()
			{
				...
				_items.Add(_productFactory.CreateRenewableMarketProduct(_itemTypeFactory.CreateResearchItem(_faction), random.Next(5,30) + extraGoods, _starId, 0, pricescale));
				...
				_items.Add(_productFactory.CreateRenewableMarketProduct(_itemTypeFactory.CreateShipItem(new CommonShip(ship)), 1, _starId, Market.ShipRenewalTime, 2f*pricescale));
				
				for (var i = 0; i < 5 + extraGoods; ++i)
                    _items.Add(_productFactory.CreateRandomComponentProduct(_starId, i, _level + 50, Constructor.ComponentQuality.P4, _faction, false, Market.CommonComponentRenewalTime, false, 1.25f*pricescale));
