...
namespace ViewModel
{
	public class StatsPanelViewModel : MonoBehaviour
	{
		...
		public TextFieldViewModel RepairRateCooldown;

		public TextFieldViewModel Energy;
		public TextFieldViewModel RechargeRate;
		public TextFieldViewModel RechargeRateCooldown;

        public TextFieldViewModel Structure;
        public TextFieldViewModel StructureRepairRate;
        public TextFieldViewModel StructureRepairRateCooldown;

        public TextFieldViewModel Shield;
        public TextFieldViewModel ShieldRechargeRate;
        public TextFieldViewModel ShieldRechargeRateCooldown;
		...
		public TextFieldViewModel StructureDamageResistance;
        ...
        public void UpdateStats(Constructor.IShipSpecification spec)
		{
			...
			RepairRateCooldown.gameObject.SetActive(spec.Stats.ArmorRepairCooldown > 0);
			RepairRateCooldown.Value.text = spec.Stats.ArmorRepairCooldown.ToString("N1");

            Structure.gameObject.SetActive(spec.Stats.StructurePoints > 0);
            Structure.Value.text = spec.Stats.StructurePoints.ToString("N1");
			StructureRepairRate.gameObject.SetActive(spec.Stats.StructureRepairRate > 0 && spec.Stats.StructurePoints > 0);
            StructureRepairRate.Value.text = spec.Stats.StructureRepairRate.ToString("N");
			StructureRepairRateCooldown.gameObject.SetActive(spec.Stats.StructureRepairCooldown > 0 && spec.Stats.StructurePoints > 0);
			StructureRepairRateCooldown.Value.text = spec.Stats.StructureRepairCooldown.ToString("N");

            Shield.gameObject.SetActive(spec.Stats.ShieldPoints > 0);
            Shield.Value.text = spec.Stats.ShieldPoints.ToString("N1");
			ShieldRechargeRate.gameObject.SetActive(spec.Stats.ShieldRechargeRate > 0 && spec.Stats.ShieldPoints > 0);
            ShieldRechargeRate.Value.text = spec.Stats.ShieldRechargeRate.ToString("N");
			ShieldRechargeRateCooldown.gameObject.SetActive(spec.Stats.ShieldRechargeCooldown > 0 && spec.Stats.ShieldPoints > 0);
			ShieldRechargeRateCooldown.Value.text = spec.Stats.ShieldRechargeCooldown.ToString("N");
			...
			RechargeRateCooldown.gameObject.SetActive(spec.Stats.EnergyRechargeCooldown > 0);
			RechargeRateCooldown.Value.text = spec.Stats.EnergyRechargeCooldown.ToString("N");
            ...
            StructureDamageResistance.gameObject.SetActive(!Mathf.Approximately(spec.Stats.StructureResistance, 0));
            StructureDamageResistance.Value.text = FloatToString(spec.Stats.StructureResistance) + " ( " + Mathf.RoundToInt(spec.Stats.StructureResistancePercentage*100) + "% )";
